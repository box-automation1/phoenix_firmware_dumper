...
if [[ -s "${PROJECT_DIR}"/.gitlab_token ]]; then
	...
	if [[ -s "${PROJECT_DIR}"/.gitlab_instance ]]; then GITLAB_INSTANCE=$(< "${PROJECT_DIR}"/.gitlab_instance) && export LAB_CORE_HOST="https://$GITLAB_INSTANCE" ; else export LAB_CORE_HOST=https://gitlab.com ; fi
	...
	if [[ -s "${PROJECT_DIR}"/.gitlab_orgname ]]; then
		GIT_ORG=$(< "${PROJECT_DIR}"/.gitlab_orgname)	# Set Your GitLab Organization Name
	else
		GIT_USER="$(git config --get user.name)"
		GIT_ORG="${GIT_USER}"				# Otherwise, Your Username will be used
	fi
	# Check if already dumped or not
	curl -sf "https://gitlab.com/${GIT_ORG}/${REPO}/-/raw/${branch}/all_files.txt" 2>/dev/null && { printf "Firmware already dumped!\nGo to https://gitlab.com/${GIT_ORG}/${repo}/-/tree/${branch}\n" && exit 1; }
	...
	git remote add origin https://gitlab.com/${GIT_ORG}/${repo}.git
	git commit -asm "Add ${description}"
	{ [[ $(du -bs .) -lt 1288490188 ]] && git push https://${GIT_USER}:${GITLAB_TOKEN}@gitlab.com/${GIT_ORG}/${repo}.git "${branch}"; } || (
		git update-ref -d HEAD
		git reset system/ vendor/
		git checkout -b "${branch}"
		git commit -asm "Add extras for ${description}"
		git push https://${GIT_USER}:${GITLAB_TOKEN}@gitlab.com/${GIT_ORG}/${repo}.git "${branch}"
		git add vendor/
	...
	if [[ -s "${PROJECT_DIR}"/.tg_token ]]; then
		...
		printf "<b>Brand: %s</b>" "${brand}" >| "${OUTDIR}"/tg.html
		{
			...
			printf "\n<a href=\"https://gitlab.com/%s/%s/-/tree/%s/\">GitLab Tree</a>" "${GIT_ORG}" "${repo}" "${branch}"
